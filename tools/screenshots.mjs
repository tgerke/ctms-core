// Regenerates the app screenshots embedded in the docs site
// (site/src/assets/screenshots/). Drives headless Chrome over the DevTools
// protocol; needs no dependencies beyond Node 22+ (built-in WebSocket) and
// Chrome at the macOS path below.
//
// The stack must be running with seeded data:  pnpm db:up && pnpm db:seed && pnpm dev
// Then:  node tools/screenshots.mjs
//
...
// server on 5173): WEB=http://localhost:5199 node tools/screenshots.mjs
const WEB = process.env.WEB ?? "http://localhost:5173";
const API = process.env.API ?? "http://localhost:8787";
const TOKEN = "dev-admin-token";
const outdir = join(dirname(fileURLToPath(import.meta.url)), "..", "site", "src", "assets", "screenshots");
