...
![Issues on the dashboard, with severity and derived status; recording a new deviation is a form on the same page (or one `POST`), and the [user guide](user-guide/issues.qmd) walks it.](images/issues.png){.screenshot fig-alt="Issues and deviations list showing severity badges (minor, major, critical) and derived status badges (open, overdue, resolved) with a form for recording a new issue"}
