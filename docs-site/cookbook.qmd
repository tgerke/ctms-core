...
<http://localhost:8787/docs>. The point-and-click versions of these same tasks
are in the [user guide](user-guide/index.qmd).
...
it effective. All three are Part 11 signatures — signer, timestamp, meaning,
bound to the content hash.
...
permission as approving, but it isn't a signature — no re-authentication.
...
When requirement rules or staff role assignments change (today that means
SQL — see the [roadmap](roadmap.qmd)), an admin re-materializes the
expected-document placeholders. The sync is idempotent: it inserts what's
newly expected and prunes unfulfilled placeholders that no longer apply.
...
the hash recorded on the signature row.
