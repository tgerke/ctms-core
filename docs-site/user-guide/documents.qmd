...
![Search results: metadata and content matches together, with a snippet where the match came from inside the file.](../images/search-results.png){.screenshot fig-alt="Document search page showing results for a two-word query, each with its TMF artifact, site, status chip, and an italic snippet of the matching text from inside the file"}
...
bound to, not a thumbnail or a converted copy. One preview is open at a
time, so working through the queue reads like paging through the stack;
PDFs, images, and plain text render inline, and anything else offers a
download. Close it, tick the box, move on.
...
![The queue with a selection and the series-signing confirmation open](../images/queue-bulk-review.png)
