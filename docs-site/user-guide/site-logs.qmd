...
![The site seat: one site, its logs, its documents, and nothing else in the header.](../images/site-seat.png){.screenshot fig-alt="Site page rendered for a site-scoped user, showing the staff roster, delegation of authority log, and training log, with study-wide navigation absent from the header"}
...
![The delegation log: tasks, dates, the authorizing PI, and the checks the record makes possible.](../images/delegation-log.png){.screenshot fig-alt="Delegation of authority log showing delegated tasks per person with start dates, authorizing investigator, an open credential item warning, and active and ended status chips"}
...
![The training log: completions with derived expiry status.](../images/training-log.png){.screenshot fig-alt="Training log showing training topics per person with completion dates, expiry dates, and current, expiring soon, and expired status chips"}
