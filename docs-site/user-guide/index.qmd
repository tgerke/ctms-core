...
![The portfolio: every study's completeness, attention items, and enrollment, computed by the same views the study pages read.](../images/portfolio.png){.screenshot fig-alt="Portfolio page showing two studies as cards, each with expected document counts, percent current, missing, expired, review queue, open issues, and enrollment totals"}
