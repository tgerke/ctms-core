...
pasted straight into an email or a chat.
...
- A **site page** (click any site) holds the site's staff roster — each
  person with their role and a count of their open document items — plus
  visits, issues, enrollment reporting, and the site's expected documents
  grouped by TMF zone, each with an upload button where one is needed.
...
limited to one study or one site. The app shows the same pages to everyone —
what the role controls is *actions*, checked at the moment you act. If you try
something your role doesn't allow, you'll see a permission message; nothing is
lost.
...
Two of these deserve a note. **Read-only** is how an auditor or inspector is
given the run of the record without any risk of changing it. **Ingest** is for
