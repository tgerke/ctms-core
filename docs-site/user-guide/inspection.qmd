...
![The binder: the study in the reference model's own order, gaps included.](../images/binder.png){.screenshot fig-alt="TMF binder page showing zones, sections, and artifacts in reference model order, with filed documents, their statuses, and missing and waived counts per artifact"}
...
![Verify bytes: the hash recomputed in the browser, compared against the record and its signatures.](../images/verify-bytes.png){.screenshot fig-alt="Document page version row after clicking Verify bytes, showing that the SHA-256 recomputed in the browser matches the recorded content hash and the signature bound to it"}
...
package; see [operations](../operations.qmd) for both.
