...
function syntheticWorkbook(): ExcelJS.Workbook {
  ...
  sheet.addRow(["Trial Master File Reference Model"]); // title above header
  sheet.addRow([
    "Zone #",
    "Zone Name",
    "Section #",
  ...
  sheet.addRow([98, "Synthetic Zone", "98.01", "Synthetic Section One", "98.01.01", "Synthetic Artifact A", "Testing"]);
  // merged-cell style: zone/section names blank on continuation rows
  sheet.addRow(["", "", "", "", "98.01.02", "Synthetic Artifact B", ""]);
...
describe("TMF RM importer", () => {
  it("finds the header row, carries merged names forward, skips non-artifacts", () => {
    const { rows, skipped, sheetName } = parseWorkbook(syntheticWorkbook());
  ...
  it("upserts idempotently: re-import updates names in place, no duplicates", async () => {
    ...
    const [artifact] = await sql`
      SELECT a.name, s.code AS section_code FROM tmf_artifact a
