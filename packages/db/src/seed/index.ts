...
import { makePdf, makeScannedPdf } from "./pdf.js";
...
interface DocSpec {
  ...
  // Office-format bytes instead of PDF (ADR-0030 renditions need real files),
  // or an image-only "scanned" PDF with no text layer (ADR-0031 OCR needs one).
  format?: "docx" | "xlsx" | "scanned";
  sheet?: { name: string; rows: (string | number)[][] }; // xlsx only
  extraLines?: string[]; // body text beyond the standard header lines
}
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    const bytes =
      spec.format === "docx"
        ...
        : spec.format === "xlsx"
          ? await makeXlsx(spec.sheet ?? { name: "Sheet1", rows: lines.map((l) => [l]) }, lines)
          : spec.format === "scanned"
            ? makeScannedPdf(lines)
            : makePdf(lines);
    ...
    await db.insert(s.documentVersion).values({
      ...
      versionNumber: i + 1,
      sha256,
      fileName: `${spec.title.replace(/[^A-Za-z0-9]+/g, "_")}_${labels[i]}.${spec.format === "docx" || spec.format === "xlsx" ? spec.format : "pdf"}`,
      mimeType: spec.format === "docx" || spec.format === "xlsx" ? OFFICE_MIME[spec.format] : "application/pdf",
      sizeBytes,
...
// Office formats awaiting review, so the queue demos in-browser renditions (ADR-0030).
await addDoc({ artifact: "01.01.02", title: "Trial Management Plan CORC-2201", status: "pending_review", format: "docx", uploadedBy: "patel" });
await addDoc({
  artifact: "08.01.02",
  title: "Laboratory Normal Ranges — UMC Central Laboratory",
  site: "001",
...
// A paper approval letter the coordinator scanned: image-only, no text layer,
// so its body is searchable only after the OCR pass below (ADR-0031).
const docIrbRenewal002 = await addDoc({
  artifact: "04.01.02",
  title: "IRB Continuing Review Approval — Site 002",
  site: "002",
  status: "pending_review",
  uploadedBy: "oduya",
  format: "scanned",
  extraLines: [
    "Scanned copy of the signed approval letter.",
    "The Board conducted continuing review of the protocol",
    "and renewed approval for a further twelve months.",
  ],
});
...
// OCR the scanned fixture (ADR-0031) so the demo can search words that exist
// only as pixels. First run downloads the eng traineddata; if that fails
// (offline), the seed still completes — `pnpm db:extract-text` retries.
try {
  const ocrCounts = await backfillOcr(sql);
  console.log(
    `ocr: ${ocrCounts.recognized} recognized, ${ocrCounts.blank} blank, ${ocrCounts.failed} failed`,
  );
} catch (err) {
