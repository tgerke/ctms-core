...
### Review outcomes beyond approval

A `pending_review` version can be approved (signature with `approval`
meaning) or left pending — there is no return-for-correction, no rejection
with a documented reason, and no QC checklist. Veeva runs pre- and
post-approval QC with tracked quality issues and annotation-based review
workflows. The absence bites in daily use: a reviewer who spots a wrong file
today has to leave it pending and chase the uploader out of band. The data
model wants a review-outcome fact (who, when, why) with derived status
picking it up — same pattern as everything else (ADR-0004, ADR-0006).
...
1. **Return-for-correction on review.** Smallest gap with daily-use impact;
   completes the document QC loop and is one fact table + one view change.
2. **Site and staff administration.** A pilot cannot onboard a site without
   SQL today; this gates any real deployment more than any other item.
3. **Expiry/overdue digest notifications.** Highest oversight value per unit
   effort — the views already compute everything; a scheduled job formats
   and sends.

Expected-document waivers ride along naturally with item 2 (both are
requirement-level administration).
