...
- **TMF transfer and inspection export** — shipped as a verifiable package
  ...
  source library — ADR-0012); what remains is purpose-built in-app auditor
  UX.
...
- **Bulk review** — shipped as a series of signings (ADR-0026): checkboxes
  ...
  single-document ceremony. What remains is reviewer ergonomics beyond the
  ceremony itself — inline document preview from the queue, so diligence
  gets as cheap as the signature did.
...
Nothing is queued: bulk review, the last candidate pulled from the "what
remains" notes, shipped 2026-07-12 (ADR-0026). Remaining candidates live in
the notes above — auditor UX, queue-side document preview, site-side depth,
