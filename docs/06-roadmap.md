...
- **Queue-side document preview** — shipped as a scoped read of the signed
  ...
  ceremony. What remains is in-browser rendering for office formats (they
  preview as a download offer); the broader auditor UX closed as ADR-0028.
- **In-app auditor UX** — shipped as a rendering of the record (ADR-0028):
  ...
  works a clean surface instead of collecting 403s. What remains is what
  ADR-0027 left: office-format renditions, OCR, relevance ranking.
...
Nothing is queued: in-app auditor UX, the last candidate pulled from the
"what remains" notes, shipped 2026-07-12 (ADR-0028). Remaining candidates
live in the notes above — site-side depth, cross-study analytics,
office-format preview renditions.
