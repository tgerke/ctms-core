...
Environment: commit 2873b7e, node v23.11.0, 2026-07-31T15:35:24.276Z

Suite result: **PASSED** — 184/184 tests passed.

## packages/core/src/digest.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | digest notifications (ADR-0017) collects a digest whose numbers cohere with the views | 22 |
| PASS | digest notifications (ADR-0017) renders a subject and body that carry the study and the counts | 9 |
...
| PASS | exchange.xml against the seeded study serializes the full study and validates against the official XSD | 57 |

## packages/core/src/engine.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | requirement engine materializes site- and person-scoped placeholders when scope appears | 49 |
...
| PASS | upload -> sign lifecycle lands pending, becomes effective on approval, binds signature to hash (§11.50 §11.70) | 54 |

## packages/core/src/export.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | TMF export (ADR-0020) collects every document with versions, signatures, and returns intact | 35 |
| PASS | TMF export (ADR-0020) every referenced blob exists and hashes to its recorded sha256 | 39 |
| PASS | TMF export (ADR-0020) carries the whole audit trail with a verified chain and its head hash | 18 |
| PASS | TMF export (ADR-0020) the expected-document snapshot matches the live view | 20 |

## packages/core/src/operations.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | monitoring visit lifecycle (derived, never stored) walks scheduled -> overdue -> awaiting_report -> report_pending_review -> follow_up -> complete | 113 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | extractContentText (ADR-0022) extracts the text of a PDF | 47 |
...
| PASS | OCR of image-only PDFs (ADR-0031) recovers text that exists only as pixels | 479 |
...
| PASS | TMF RM importer upserts idempotently: re-import updates names in place, no duplicates | 38 |
...
| PASS | s3 driver (MinIO, Object Lock) s3 driver contract stores content-addressed, round-trips bytes, reports presence | 48 |
| PASS | s3 driver (MinIO, Object Lock) WORM (§11.10(c)): a locked object version cannot be deleted, even by the root credential | 18 |

## apps/api/src/admin.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | site onboarding (ADR-0016) onboards a site end to end: org → site → study-site → activate → staff → sync | 46 |
| PASS | site onboarding (ADR-0016) duplicate site number on the study is refused | 6 |
| PASS | site onboarding (ADR-0016) ending a role is a dated fact, not a delete | 4 |
| PASS | site onboarding (ADR-0016) admin mutations are attributed in the audit trail | 1 |
| PASS | site onboarding (ADR-0016) the monitor role gets 403 on every admin mutation | 5 |
| PASS | site onboarding (ADR-0016) grants and revokes access; revocation is a fact, revoking twice refuses | 19 |
...
| PASS | expected-document waivers (ADR-0016) a filed document beats the waiver | 62 |
| PASS | expected-document waivers (ADR-0016) lifting the waiver restores 'missing' and keeps the history | 9 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | GET /studies/{id}/binder (ADR-0028) serves the taxonomy in reference-model order with filed documents attached | 38 |
| PASS | GET /studies/{id}/binder (ADR-0028) rolls up expected-document status per artifact from the same view | 36 |
| PASS | GET /studies/{id}/binder (ADR-0028) is a study-scoped read: the site seat gets 403 | 2 |
| PASS | the auditor's seat: unscoped read_only (ADR-0028) /me names the person and the single unscoped read_only grant | 3 |
| PASS | the auditor's seat: unscoped read_only (ADR-0028) reads the whole record: studies, binder, portfolio, audit trail, chain, bytes | 118 |
| PASS | the auditor's seat: unscoped read_only (ADR-0028) cannot change anything (§11.10(g)): upload, sign, bulk-approve, grant, sync all 403 | 10 |
...
| PASS | authorization (§11.10(g), ADR-0008) denies operations the role does not include, naming the permission | 9 |
| PASS | authorization (§11.10(g), ADR-0008) allows reads for every seeded role | 12 |
| PASS | authorization (§11.10(g), ADR-0008) denies approval signatures to the monitor role but allows review | 64 |
...
| PASS | signing re-authentication (§11.200) records the re-auth method and time on the signature row | 12 |
...
| PASS | OIDC authentication (§11.10(d)) rejects a forged token (wrong key) | 31 |
| PASS | OIDC authentication (§11.10(d)) rejects an authenticated identity with no person record (403, not a fallback actor) | 2 |
| PASS | OIDC authentication (§11.10(d)) rejects a token whose email is explicitly unverified | 1 |
| PASS | OIDC signing re-authentication (§11.200) accepts a fresh re-auth token for the same subject and records it | 75 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | machine identity (ADR-0011) authenticates a configured service subject with no email claim | 21 |
| PASS | machine identity (ADR-0011) still rejects an unconfigured subject with no email claim | 2 |
| PASS | machine identity (ADR-0011) files a document with provenance, attributed to the service actor | 49 |
| PASS | machine identity (ADR-0011) cannot sign: ingest grants upload but no signing ceremony | 10 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | bulk approval (ADR-0026) one re-authentication opens the series; each version gains its own signature bound to its own hash (§11.200 §11.70) | 111 |
| PASS | bulk approval (ADR-0026) refuses the whole selection with every blocker listed, signing nothing | 37 |
| PASS | bulk approval (ADR-0026) requires approve authority (a monitor holds sign, not approve) | 7 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | GET /document-versions/{id}/content (ADR-0027) returns the exact bytes with the uploaded mime type, file name, and hash (§11.10(b)) | 27 |
| PASS | GET /document-versions/{id}/content (ADR-0027) requires authentication | 8 |
| PASS | GET /document-versions/{id}/content (ADR-0027) is scoped to the version's site: the site seat reads its own site only | 13 |
| PASS | GET /document-versions/{id}/content (ADR-0027) study-wide read reaches every site's documents | 8 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | EMS import surface (ADR-0025) GET /tmf-artifacts carries the TMF RM unique ID, the EMS mapping key | 8 |
| PASS | EMS import surface (ADR-0025) GET /studies/{id}/filings starts empty for a source system that never filed | 3 |
| PASS | EMS import surface (ADR-0025) imports a partner batch through the filing endpoint as the ingest identity, idempotently | 100 |
| PASS | EMS import surface (ADR-0025) POST /documents/{id}/versions appends to exactly that document | 13 |
| PASS | EMS import surface (ADR-0025) refuses to grow a superseded document — closed history stays closed | 31 |

## apps/api/src/portfolio.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | portfolio (ADR-0021) returns one row per study, ordered by protocol number | 97 |
| PASS | portfolio (ADR-0021) rolls up the second study exactly as seeded — studies do not bleed into each other | 46 |
| PASS | portfolio (ADR-0021) is readable with any read-permitting grant | 46 |
| PASS | portfolio (ADR-0021) per-study surfaces stay scoped: 2202's expected documents are only its own | 53 |

## apps/api/src/return.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | return-for-correction (ADR-0015) records the returner, reason, and time, and moves the document to 'returned' | 88 |
...
| PASS | return-for-correction (ADR-0015) a returned version can never be approved | 14 |
| PASS | return-for-correction (ADR-0015) a corrected version reopens review, and only it can be approved | 23 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | review queue (ADR-0018) an unassigned pending version sits in the queue as 'unassigned' | 62 |
| PASS | review queue (ADR-0018) assigning with a past due date derives 'overdue'; filters find it | 16 |
| PASS | review queue (ADR-0018) reassignment inserts a new row and the latest one stands | 19 |
| PASS | review queue (ADR-0018) the assignee must be able to approve: a monitor-role assignee is refused | 10 |
| PASS | review queue (ADR-0018) assigning takes 'approve' authority: the monitor token gets 403 | 6 |
| PASS | review queue (ADR-0018) approval clears the entry from the queue — the assignment resolves itself | 19 |
| PASS | review queue (ADR-0018) a return clears the entry too, and the returned version cannot be assigned | 15 |
| PASS | review queue (ADR-0018) assignments land on the document detail and in the audit trail | 15 |

## apps/api/src/search.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | document search (ADR-0019) finds a document by title words, stem-free but case-insensitive | 17 |
| PASS | document search (ADR-0019) every token must match: adding a person narrows to their documents | 14 |
| PASS | document search (ADR-0019) matches artifact codes and site numbers ('04.01 002') | 12 |
...
| PASS | content full-text search (ADR-0022) a word that exists only inside the PDF finds the document, with a snippet | 64 |
...
| PASS | the site seat is a permission scope (ADR-0023) reads its own site: overview, expected documents, enrollment, staff | 24 |
| PASS | the site seat is a permission scope (ADR-0023) is refused everywhere else — other sites, study-wide reads, the portfolio (§11.10(g)) | 6 |
| PASS | delegation-of-authority log (ADR-0023) site staff records a delegation; the view derives active + PI check | 19 |
| PASS | delegation-of-authority log (ADR-0023) an authorizer who never held the PI role is flagged, not refused | 7 |
| PASS | delegation-of-authority log (ADR-0023) refuses self-delegation, empty tasks, and monitor authorship (§11.10(g)) | 7 |
| PASS | delegation-of-authority log (ADR-0023) ending is a dated fact; ending twice refuses | 9 |
| PASS | delegation-of-authority log (ADR-0023) log writes land in the audit trail attributed to the site persona | 2 |
| PASS | training log (ADR-0023) records a completion and derives expiry status | 6 |
| PASS | training log (ADR-0023) refuses a blank topic and an expiry before completion | 5 |
| PASS | training log (ADR-0023) oversight reads the log; the monitor cannot write it | 3 |

## apps/api/src/study-startup.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | study creation (ADR-0034) creates a study in planning with template rules cloned and expected documents materialized | 36 |
...
| PASS | study creation (ADR-0034) rejects a duplicate protocol number | 3 |
| PASS | study creation (ADR-0034) rejects an unknown template study and an unknown sponsor | 5 |
| PASS | guided startup (ADR-0034) derives the startup checklist from facts, never state | 43 |
...
| PASS | renderRendition docx converts paragraphs to a self-contained HTML document | 56 |
| PASS | renderRendition docx escapes markup smuggled in document text | 4 |
| PASS | renderRendition docx rejects bytes that are not a docx | 2 |
| PASS | renderRendition xlsx renders every sheet as a named table | 38 |
| PASS | renderRendition xlsx escapes markup smuggled in cells and sheet names | 5 |
