...
Environment: commit d4a2257, node v22.23.1, 2026-07-12T16:56:12.597Z

Suite result: **PASSED** — 123/123 tests passed.

## packages/core/src/digest.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | digest notifications (ADR-0017) collects a digest whose numbers cohere with the views | 31 |
| PASS | digest notifications (ADR-0017) renders a subject and body that carry the study and the counts | 12 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | requirement engine materializes site- and person-scoped placeholders when scope appears | 85 |
| PASS | requirement engine is idempotent | 6 |
| PASS | derived status (ADR-0004) derives expired and expiring_soon from effective_date + validity | 19 |
| PASS | upload -> sign lifecycle lands pending, becomes effective on approval, binds signature to hash (§11.50 §11.70) | 124 |

## packages/core/src/export.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | TMF export (ADR-0020) collects every document with versions, signatures, and returns intact | 39 |
| PASS | TMF export (ADR-0020) every referenced blob exists and hashes to its recorded sha256 | 34 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | monitoring visit lifecycle (derived, never stored) walks scheduled -> overdue -> awaiting_report -> report_pending_review -> follow_up -> complete | 163 |
...
| PASS | least-privilege runtime role (§11.10(c) §11.10(d)) keeps immutability guarantees (UPDATE/DELETE rejected by trigger) | 4 |
...
| PASS | s3 driver (MinIO, Object Lock) s3 driver contract stores content-addressed, round-trips bytes, reports presence | 87 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | site onboarding (ADR-0016) onboards a site end to end: org → site → study-site → activate → staff → sync | 87 |
...
| PASS | expected-document waivers (ADR-0016) a filed document beats the waiver | 86 |
...
| PASS | authorization (§11.10(g), ADR-0008) allows reads for every seeded role | 20 |
| PASS | authorization (§11.10(g), ADR-0008) denies approval signatures to the monitor role but allows review | 107 |
...
| PASS | signing re-authentication (§11.200) rejects a signature without valid re-authentication | 12 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | OIDC authentication (§11.10(d)) accepts a valid token and resolves the person by email claim | 71 |
| PASS | OIDC authentication (§11.10(d)) rejects a token for the wrong audience | 4 |
| PASS | OIDC authentication (§11.10(d)) rejects a forged token (wrong key) | 253 |
| PASS | OIDC authentication (§11.10(d)) rejects an authenticated identity with no person record (403, not a fallback actor) | 11 |
| PASS | OIDC authentication (§11.10(d)) rejects a token whose email is explicitly unverified | 3 |
| PASS | OIDC signing re-authentication (§11.200) accepts a fresh re-auth token for the same subject and records it | 319 |
| PASS | OIDC signing re-authentication (§11.200) rejects a stale re-auth token (auth_time outside the freshness window) | 150 |
| PASS | OIDC signing re-authentication (§11.200) rejects a re-auth token minted for a different subject | 47 |

## apps/api/src/auth-service.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | machine identity (ADR-0011) authenticates a configured service subject with no email claim | 67 |
| PASS | machine identity (ADR-0011) still rejects an unconfigured subject with no email claim | 4 |
| PASS | machine identity (ADR-0011) files a document with provenance, attributed to the service actor | 95 |
| PASS | machine identity (ADR-0011) cannot sign: ingest grants upload but no signing ceremony | 23 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | portfolio (ADR-0021) returns one row per study, ordered by protocol number | 973 |
| PASS | portfolio (ADR-0021) rolls up the second study exactly as seeded — studies do not bleed into each other | 653 |
| PASS | portfolio (ADR-0021) is readable with any read-permitting grant | 655 |
| PASS | portfolio (ADR-0021) per-study surfaces stay scoped: 2202's expected documents are only its own | 669 |

## apps/api/src/return.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | return-for-correction (ADR-0015) records the returner, reason, and time, and moves the document to 'returned' | 148 |
...
| PASS | return-for-correction (ADR-0015) a corrected version reopens review, and only it can be approved | 45 |
| PASS | return-for-correction (ADR-0015) only a pending_review document can be returned | 7 |
| PASS | return-for-correction (ADR-0015) only the latest version can be returned | 28 |
| PASS | return-for-correction (ADR-0015) the return lands in the document's audit trail | 28 |

## apps/api/src/review-queue.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | review queue (ADR-0018) an unassigned pending version sits in the queue as 'unassigned' | 114 |
| PASS | review queue (ADR-0018) assigning with a past due date derives 'overdue'; filters find it | 35 |
| PASS | review queue (ADR-0018) reassignment inserts a new row and the latest one stands | 33 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | document search (ADR-0019) finds a document by title words, stem-free but case-insensitive | 40 |
| PASS | document search (ADR-0019) every token must match: adding a person narrows to their documents | 28 |
...
| PASS | content full-text search (ADR-0022) a word that exists only inside the PDF finds the document, with a snippet | 104 |
| PASS | content full-text search (ADR-0022) tokens mix freely across metadata and content | 7 |
| PASS | content full-text search (ADR-0022) a metadata-only match carries no snippet | 7 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | the site seat is a permission scope (ADR-0023) /me names the person and the site-scoped grant | 20 |
| PASS | the site seat is a permission scope (ADR-0023) reads its own site: overview, expected documents, enrollment, staff | 36 |
| PASS | the site seat is a permission scope (ADR-0023) is refused everywhere else — other sites, study-wide reads, the portfolio (§11.10(g)) | 10 |
...
| PASS | delegation-of-authority log (ADR-0023) ending is a dated fact; ending twice refuses | 17 |
