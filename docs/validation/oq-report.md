...
Environment: commit 54be7c9, node v22.23.1, 2026-07-12T23:41:50.087Z

Suite result: **PASSED** — 154/154 tests passed.

## packages/core/src/digest.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | digest notifications (ADR-0017) collects a digest whose numbers cohere with the views | 25 |
| PASS | digest notifications (ADR-0017) renders a subject and body that carry the study and the counts | 9 |
| PASS | digest notifications (ADR-0017) a broken chain leads the email and the subject count | 8 |
...
| PASS | exchange.xml against the seeded study serializes the full study and validates against the official XSD | 30 |

## packages/core/src/engine.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | requirement engine materializes site- and person-scoped placeholders when scope appears | 75 |
...
| PASS | upload -> sign lifecycle lands pending, becomes effective on approval, binds signature to hash (§11.50 §11.70) | 60 |
...
| PASS | TMF export (ADR-0020) every referenced blob exists and hashes to its recorded sha256 | 32 |
| PASS | TMF export (ADR-0020) carries the whole audit trail with a verified chain and its head hash | 14 |
| PASS | TMF export (ADR-0020) the expected-document snapshot matches the live view | 12 |

## packages/core/src/operations.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | monitoring visit lifecycle (derived, never stored) walks scheduled -> overdue -> awaiting_report -> report_pending_review -> follow_up -> complete | 117 |
...
| PASS | issue lifecycle (derived) derives open, overdue, and resolved from dated facts | 6 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | extractContentText (ADR-0022) extracts the text of a PDF | 53 |
...
| PASS | audit trail (§11.10(e)) writes an attributed, chained event for every domain mutation | 5 |
| PASS | audit trail (§11.10(e)) verifies clean on untampered data | 3 |
| PASS | audit trail (§11.10(e)) detects tampering when a row is altered with triggers disabled | 5 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | least-privilege runtime role (§11.10(c) §11.10(d)) cannot TRUNCATE domain tables | 23 |
...
| PASS | s3 driver (MinIO, Object Lock) s3 driver contract stores content-addressed, round-trips bytes, reports presence | 110 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | site onboarding (ADR-0016) onboards a site end to end: org → site → study-site → activate → staff → sync | 65 |
| PASS | site onboarding (ADR-0016) duplicate site number on the study is refused | 5 |
| PASS | site onboarding (ADR-0016) ending a role is a dated fact, not a delete | 5 |
...
| PASS | site onboarding (ADR-0016) grants and revokes access; revocation is a fact, revoking twice refuses | 13 |
...
| PASS | expected-document waivers (ADR-0016) a filed document beats the waiver | 49 |
...
| PASS | authorization (§11.10(g), ADR-0008) denies approval signatures to the monitor role but allows review | 62 |
...
| PASS | OIDC signing re-authentication (§11.200) accepts a fresh re-auth token for the same subject and records it | 88 |
| PASS | OIDC signing re-authentication (§11.200) rejects a stale re-auth token (auth_time outside the freshness window) | 15 |
| PASS | OIDC signing re-authentication (§11.200) rejects a re-auth token minted for a different subject | 12 |

## apps/api/src/auth-service.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | machine identity (ADR-0011) authenticates a configured service subject with no email claim | 23 |
| PASS | machine identity (ADR-0011) still rejects an unconfigured subject with no email claim | 2 |
| PASS | machine identity (ADR-0011) files a document with provenance, attributed to the service actor | 57 |
| PASS | machine identity (ADR-0011) cannot sign: ingest grants upload but no signing ceremony | 15 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | bulk approval (ADR-0026) one re-authentication opens the series; each version gains its own signature bound to its own hash (§11.200 §11.70) | 107 |
| PASS | bulk approval (ADR-0026) refuses the whole selection with every blocker listed, signing nothing | 37 |
| PASS | bulk approval (ADR-0026) requires approve authority (a monitor holds sign, not approve) | 5 |
| PASS | bulk approval (ADR-0026) refuses the series without valid re-authentication (§11.200) | 7 |
| PASS | bulk return (ADR-0026 over ADR-0015) returns the selection with one shared immutable reason | 22 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | EMS import surface (ADR-0025) GET /tmf-artifacts carries the TMF RM unique ID, the EMS mapping key | 11 |
| PASS | EMS import surface (ADR-0025) GET /studies/{id}/filings starts empty for a source system that never filed | 7 |
| PASS | EMS import surface (ADR-0025) imports a partner batch through the filing endpoint as the ingest identity, idempotently | 113 |
| PASS | EMS import surface (ADR-0025) POST /documents/{id}/versions appends to exactly that document | 16 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | portfolio (ADR-0021) returns one row per study, ordered by protocol number | 505 |
| PASS | portfolio (ADR-0021) rolls up the second study exactly as seeded — studies do not bleed into each other | 385 |
| PASS | portfolio (ADR-0021) is readable with any read-permitting grant | 410 |
| PASS | portfolio (ADR-0021) per-study surfaces stay scoped: 2202's expected documents are only its own | 428 |

## apps/api/src/return.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | return-for-correction (ADR-0015) records the returner, reason, and time, and moves the document to 'returned' | 98 |
| PASS | return-for-correction (ADR-0015) requires 'approve' permission: the monitor role gets 403 | 10 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | review queue (ADR-0018) an unassigned pending version sits in the queue as 'unassigned' | 71 |
| PASS | review queue (ADR-0018) assigning with a past due date derives 'overdue'; filters find it | 35 |
| PASS | review queue (ADR-0018) reassignment inserts a new row and the latest one stands | 24 |
...
| PASS | review queue (ADR-0018) a return clears the entry too, and the returned version cannot be assigned | 16 |
| PASS | review queue (ADR-0018) assignments land on the document detail and in the audit trail | 19 |

## apps/api/src/search.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | document search (ADR-0019) finds a document by title words, stem-free but case-insensitive | 19 |
| PASS | document search (ADR-0019) every token must match: adding a person narrows to their documents | 14 |
| PASS | document search (ADR-0019) matches artifact codes and site numbers ('04.01 002') | 5 |
| PASS | document search (ADR-0019) filters by document status | 5 |
| PASS | document search (ADR-0019) LIKE wildcards in the query are literals, not injection | 10 |
...
| PASS | content full-text search (ADR-0022) a word that exists only inside the PDF finds the document, with a snippet | 70 |
| PASS | content full-text search (ADR-0022) tokens mix freely across metadata and content | 6 |
| PASS | content full-text search (ADR-0022) a metadata-only match carries no snippet | 5 |
| PASS | content full-text search (ADR-0022) unextractable bytes never block the upload; the failure is recorded | 11 |

## apps/api/src/site-seat.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | the site seat is a permission scope (ADR-0023) /me names the person and the site-scoped grant | 9 |
| PASS | the site seat is a permission scope (ADR-0023) reads its own site: overview, expected documents, enrollment, staff | 28 |
...
| PASS | delegation-of-authority log (ADR-0023) site staff records a delegation; the view derives active + PI check | 22 |
| PASS | delegation-of-authority log (ADR-0023) an authorizer who never held the PI role is flagged, not refused | 8 |
| PASS | delegation-of-authority log (ADR-0023) refuses self-delegation, empty tasks, and monitor authorship (§11.10(g)) | 9 |
| PASS | delegation-of-authority log (ADR-0023) ending is a dated fact; ending twice refuses | 10 |
| PASS | delegation-of-authority log (ADR-0023) log writes land in the audit trail attributed to the site persona | 2 |
| PASS | training log (ADR-0023) records a completion and derives expiry status | 8 |
