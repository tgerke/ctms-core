...
Environment: commit 2f1e8be, node v22.23.1, 2026-07-12T16:14:31.473Z

Suite result: **PASSED** — 112/112 tests passed.
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | append-only enforcement (Part 11 §11.10(c) §11.10(e)) rejects UPDATE on audit_event at the database level | 34 |
...
| PASS | audit trail (§11.10(e)) writes an attributed, chained event for every domain mutation | 10 |
...
| PASS | TMF RM importer upserts idempotently: re-import updates names in place, no duplicates | 54 |
...
| PASS | least-privilege runtime role (§11.10(c) §11.10(d)) cannot write audit_event directly, yet its DML is still audited | 14 |
...
| PASS | s3 driver (MinIO, Object Lock) s3 driver contract stores content-addressed, round-trips bytes, reports presence | 64 |
...
| PASS | upload -> sign lifecycle lands pending, becomes effective on approval, binds signature to hash (§11.50 §11.70) | 101 |
...
| PASS | TMF export (ADR-0020) every referenced blob exists and hashes to its recorded sha256 | 45 |
| PASS | TMF export (ADR-0020) carries the whole audit trail with a verified chain and its head hash | 20 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | monitoring visit lifecycle (derived, never stored) walks scheduled -> overdue -> awaiting_report -> report_pending_review -> follow_up -> complete | 176 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | site onboarding (ADR-0016) onboards a site end to end: org → site → study-site → activate → staff → sync | 85 |
...
| PASS | site onboarding (ADR-0016) creates and updates a requirement rule, and sync materializes it | 19 |
...
| PASS | expected-document waivers (ADR-0016) a filed document beats the waiver | 92 |
| PASS | expected-document waivers (ADR-0016) lifting the waiver restores 'missing' and keeps the history | 14 |
...
| PASS | authorization (§11.10(g), ADR-0008) allows reads for every seeded role | 19 |
| PASS | authorization (§11.10(g), ADR-0008) denies approval signatures to the monitor role but allows review | 102 |
...
| PASS | signing re-authentication (§11.200) rejects a signature without valid re-authentication | 16 |
| PASS | signing re-authentication (§11.200) records the re-auth method and time on the signature row | 23 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | OIDC authentication (§11.10(d)) accepts a valid token and resolves the person by email claim | 47 |
| PASS | OIDC authentication (§11.10(d)) rejects a token for the wrong audience | 3 |
| PASS | OIDC authentication (§11.10(d)) rejects a forged token (wrong key) | 35 |
...
| PASS | OIDC signing re-authentication (§11.200) accepts a fresh re-auth token for the same subject and records it | 132 |
| PASS | OIDC signing re-authentication (§11.200) rejects a stale re-auth token (auth_time outside the freshness window) | 26 |
...
| PASS | machine identity (ADR-0011) cannot sign: ingest grants upload but no signing ceremony | 22 |
| PASS | machine identity (ADR-0011) leaves provenance null for uploads that do not claim it | 18 |

## apps/api/src/portfolio.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | portfolio (ADR-0021) returns one row per study, ordered by protocol number | 161 |
| PASS | portfolio (ADR-0021) rolls up the second study exactly as seeded — studies do not bleed into each other | 77 |
| PASS | portfolio (ADR-0021) is readable with any read-permitting grant | 75 |
| PASS | portfolio (ADR-0021) per-study surfaces stay scoped: 2202's expected documents are only its own | 86 |

## apps/api/src/return.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | return-for-correction (ADR-0015) records the returner, reason, and time, and moves the document to 'returned' | 140 |
...
| PASS | return-for-correction (ADR-0015) a corrected version reopens review, and only it can be approved | 40 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | review queue (ADR-0018) an unassigned pending version sits in the queue as 'unassigned' | 112 |
| PASS | review queue (ADR-0018) assigning with a past due date derives 'overdue'; filters find it | 37 |
...
| PASS | review queue (ADR-0018) assigning takes 'approve' authority: the monitor token gets 403 | 10 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | document search (ADR-0019) finds a document by title words, stem-free but case-insensitive | 30 |
| PASS | document search (ADR-0019) every token must match: adding a person narrows to their documents | 23 |
...
| PASS | document search (ADR-0019) LIKE wildcards in the query are literals, not injection | 8 |
...
| PASS | content full-text search (ADR-0022) a word that exists only inside the PDF finds the document, with a snippet | 105 |
...
| PASS | content full-text search (ADR-0022) unextractable bytes never block the upload; the failure is recorded | 20 |
