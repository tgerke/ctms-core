...
| §11.10(g) authority checks | Role-based grants (`access_grant`: admin / trial_ops / monitor / read_only / ingest → read / upload / sign / approve / administer, scoped to study or site; `ingest` is the machine-identity role for source-system filing, ADR-0011) enforced per route; grant changes are themselves audited and revocation is a fact, not a delete. Actor identity bound per transaction (`ctms.actor_id`) | `packages/core/src/authz.ts`, ADR-0008 |
